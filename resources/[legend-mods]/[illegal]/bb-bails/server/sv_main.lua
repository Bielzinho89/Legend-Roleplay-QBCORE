...
RegisterServerEvent('cr-bails:load', function(machine)
    local src = source
	...
	TriggerClientEvent('cr-bails:state', -1, machine, false, false)
	...
	if num_bags < 1 then
		...
		CONFIG['Machines'][machine]['finished'] = false
		TriggerClientEvent('cr-bails:state', -1, machine, true, false)
		return
	end
	...
	TriggerClientEvent('cr-phone:client:LaunderNotify', src)

	CONFIG['Machines'][machine]['available'] = false
	CONFIG['Machines'][machine]['finished'] = true
	TriggerClientEvent('cr-bails:state', -1, machine, false, true)
end)

RegisterServerEvent('cr-bails:collect', function(machine)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
