...
RegisterNetEvent("cr-fleeca:resetDoorState")
AddEventHandler("cr-fleeca:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("cr-fleeca:lootup_c")
AddEventHandler("cr-fleeca:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("cr-fleeca:outcome")
AddEventHandler("cr-fleeca:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("cr-fleeca:startheist", Fleeca.Banks[arg], arg)
    elseif not oc then
        -- TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("cr-fleeca:startLoot_b")
AddEventHandler("cr-fleeca:startLoot_b", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("cr-fleeca:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("cr-fleeca:startLoot_c")
AddEventHandler("cr-fleeca:startLoot_c", function(data, name)
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    if not LootCheck[name].Stop then
        Citizen.CreateThread(function()
...
RegisterNetEvent("cr-fleeca:stopHeist_c")
AddEventHandler("cr-fleeca:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- Main Door
AddEventHandler("cr-fleeca:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("cr-fleeca:toggleVault")
AddEventHandler("cr-fleeca:toggleVault", function(key, state)
    dooruse = true
    if Fleeca.Banks[key].hash == nil then
        if not state then
            local obj = GetClosestObjectOfType(Fleeca.Banks[key].doors.startloc.x, Fleeca.Banks[key].doors.startloc.y, Fleeca.Banks[key].doors.startloc.z, 2.0, GetHashKey("v_ilev_gb_vauldr"), false, false, false)
            local count = 0
            ...
            TriggerServerEvent("cr-fleeca:updateVaultState", key, Doors[key][1].state)
        elseif state then
            local obj = GetClosestObjectOfType(Fleeca.Banks[key].doors.startloc.x, Fleeca.Banks[key].doors.startloc.y, Fleeca.Banks[key].doors.startloc.z, 2.0, GetHashKey("v_ilev_gb_vauldr"), false, false, false)
            local count = 0
...
AddEventHandler("cr-fleeca:reset", function(name, data)
    for i = 1, #LootCheck[name], 1 do
        LootCheck[name][i] = false
    end
    Check[name] = false
    Citizen.Wait(60000)
    TriggerServerEvent("cr-fleeca:toggleVault", name, true)
    TriggerEvent("cr-fleeca:cleanUp", data, name)
end)

AddEventHandler("cr-fleeca:startheist", function(data, name)
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent("cr-fleeca:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

AddEventHandler("cr-fleeca:cleanUp", function(data, name)
    Citizen.Wait(10000)
    ...
    TriggerServerEvent("cr-fleeca:setCooldown", name)
    initiator = false
end)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("cr-fleeca:startLoot", data, name, missionplayers)
    done = false
end
...
function StartGrab(name)
    ...
    disableinput = false
    TriggerServerEvent("cr-fleeca:rewardCash")
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("cr-fleeca:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('cr-fleeca:darkmail')
AddEventHandler('cr-fleeca:darkmail', function()
    Citizen.Wait(1000)
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = "Dark Market",
        subject = "#A-1001",
        message = "<br><br> Nice, </b><br><br> You bypassed the captcha. Give me a few moments to open the door!",
    })
    exports['qb-dispatch']:FleecaBankRobbery()
end)

RegisterNetEvent('cr-fleeca:uselaptop3')
AddEventHandler('cr-fleeca:uselaptop3', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    QBCore.Functions.TriggerCallback("cr-fleeca:getBanks", function(bank, door)
        Fleeca.Banks = bank
        Doors = door
    end)
	TriggerEvent("cr-fleeca:freezeDoors")
