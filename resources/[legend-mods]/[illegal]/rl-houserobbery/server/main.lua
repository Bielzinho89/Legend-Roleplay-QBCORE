...
Citizen.CreateThread(function()
	...
	if Config.Framework == 'ESX' then
		...
		ESX.RegisterServerCallback('rl_houserobbery:entrada', function(source, cb)		
			cb(x,y,z,num)
		end)		
		ESX.RegisterServerCallback('rl_houserobbery:lockpick', function(source,cb)
		...
		ESX.RegisterServerCallback('rl_houserobbery:trabajos', function(source,cb)
			local xPlayer = ESX.GetPlayerFromId(source)
			local identifier = xPlayer.identifier
	...
	elseif Config.Framework == 'QBCORE' then
		QBCore.Functions.CreateCallback('rl_houserobbery:entrada', function(source, cb)		
			cb(x,y,z,num)
		end)
		QBCore.Functions.CreateCallback('rl_houserobbery:lockpick', function(source,cb)
		...
		QBCore.Functions.CreateCallback('rl_houserobbery:trabajos', function(source,cb)
			local player = QBCore.Functions.GetPlayer(source)
	...
	elseif Config.Framework == 'VRP' then
		...
		RegisterServerCallback('rl_houserobbery:entrada', function(source, ...)
			local cb = {x,y,z,num}
			return cb
		end)
		RegisterServerCallback('rl_houserobbery:lockpick', function(source, ...)
		...
		RegisterServerCallback('rl_houserobbery:trabajos', function(source, ...)
			local identifier = getIdenti(source)
			local status = CheckCooldownTimer(identifier)
			return status
		end)
...
RegisterServerEvent('rl_houserobbery:item')
AddEventHandler('rl_houserobbery:item', function(tipo,m)
	if m ~= num then 
		...
		if tipo == 'random' then
			...
			for i = 1, #items do
				...
				elseif items[i].type == 'item' then
					...
					TriggerClientEvent('rl_houserobbery:notification',source,Config.Lang['money_found']..items[i].itemQty)
				end
			end
	...
	elseif Config.Framework == 'QBCORE' then
		...
		if tipo == 'random' then
			...
			for i = 1, #items do
				...
				elseif items[i].type == 'item' then
					player.Functions.AddItem(items[i].itemQty)
					TriggerClientEvent('rl_houserobbery:notification',source,Config.Lang['money_found']..items[i].itemQty)
				end
			end		
		elseif tipo == 'cajafuerte' then
...
RegisterServerEvent('rl_houserobbery:policeCall')
AddEventHandler('rl_houserobbery:policeCall', function(c)
	--TriggerClientEvent('rl_houserobbery:policiablip',-1,c)
