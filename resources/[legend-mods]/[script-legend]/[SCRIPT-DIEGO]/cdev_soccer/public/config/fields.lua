...
SoccerFields = {
    -- Default Field
    {
        ...
        FieldZone = createBoxZone(vector3(771.29, -233.41, 66.11), 77.600000000001, 119.6, { -- Polyzone for the field (must be bigger than the area zone and cover the npc)
            name="default_field",
            heading=333,
            minZ=30.91,
            maxZ=95.91,
        }),
        AreaZone = createBoxZone(vector3(771.02, -233.89, 66.11), 39.4, 69.0, { -- Polyzone for the area (if ball leaves this area it's "out")
            name="default_field_area",
            heading=333,
            minZ=30.91,
            maxZ=95.91,
        }),
        GoaleeZoneRed = createBoxZone(vector3(744.67, -220.58, 66.11), 20.6, 8.8, { -- Polyzone for the red team goalee area (where the goalee can use their hands)
            name="default_field_goaleered",
            heading=334,
            minZ=65.11,
            maxZ=69.11,
          }),
        GoaleeZoneBlue = createBoxZone(vector3(798.15, -247.13, 66.11), 20.0, 7.6, { -- Polyzone for the blue team goalee area (where the goalee can use their hands)
            name="default_field_goaleeblue",
            heading=333,
            minZ=65.11,
            maxZ=69.11
        }),
        RedTeamGoalZone = createBoxZone(vector3(740.98, -218.68, 66.11), 7.4, 0.4, { -- Polyzone for the red team goal (where the ball must be to score)
            name="default_field_goalred",
            heading=334,
            minZ=64.11,
            maxZ=68.11,
        }),
        BlueTeamGoalZone = createBoxZone(vector3(801.7, -248.6, 66.11), 7.4, 0.4, { -- Polyzone for the blue team goal (where the ball must be to score)
