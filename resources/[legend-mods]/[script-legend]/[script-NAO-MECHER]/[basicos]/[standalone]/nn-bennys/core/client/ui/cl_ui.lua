...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    if vehicleHealth < 1000.0 then
        local repairCost = math.ceil(1000 - vehicleHealth) * 2

        TriggerServerEvent("nn-bennys:updateRepairCost", repairCost)
        createMenu("repairMenu", "Welcome to PD Workshop", "Repair Vehicle")
        populateMenu("repairMenu", -1, "Repair", "$" .. repairCost)
        finishPopulatingMenu("repairMenu")
    end

    --#[Main Menu]#--
    createMenu("mainMenu", "Welcome to PD Workshop", "Choose a Category")
