...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://np-tunershop/np-ui:aracdegis', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://np-tunershop/np-ui:application-closed2', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase2", function () {
    canPress = false
    $.post('https://np-tunershop/showroompurchase2CurrentVehicle2', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
