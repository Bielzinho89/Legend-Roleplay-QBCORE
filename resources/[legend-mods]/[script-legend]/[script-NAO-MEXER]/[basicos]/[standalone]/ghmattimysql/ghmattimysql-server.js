...
"use strict";
var pna=__webpack_require__(11);function destroy(err,cb){var _this=this;var readableDestroyed=this._readableState&&this._readableState.destroyed;var writableDestroyed=this._writableState&&this._writableState.destroyed;if(readableDestroyed||writableDestroyed){if(cb){cb(err);}else if(err&&(!this._writableState||!this._writableState.errorEmitted)){pna.nextTick(emitErrorNT,this,err);}return this;}if(this._readableState){this._readableState.destroyed=true;}if(this._writableState){this._writableState.destroyed=true;}this._destroy(err||null,function(err){if(!cb&&err){pna.nextTick(emitErrorNT,_this,err);if(_this._writableState){_this._writableState.errorEmitted=true;}}else if(cb){cb(err);}});return this;}function undestroy(){if(this._readableState){this._readableState.destroyed=false;this._readableState.reading=false;this._readableState.ended=false;this._readableState.endEmitted=false;}if(this._writableState){this._writableState.destroyed=false;this._writableState.ended=false;this._writableState.ending=false;this._writableState.finished=false;this._writableState.errorEmitted=false;}}function emitErrorNT(self,err){self.emit('error',err);}module.exports={destroy:destroy,undestroy:undestroy};
...
"use strict";
module.exports=Transform;var Duplex=__webpack_require__(5);var util=__webpack_require__(6);util.inherits=__webpack_require__(7);util.inherits(Transform,Duplex);function afterTransform(er,data){var ts=this._transformState;ts.transforming=false;var cb=ts.writecb;if(!cb){return this.emit('error',new Error('write callback called multiple times'));}ts.writechunk=null;ts.writecb=null;if(data!=null)this.push(data);cb(er);var rs=this._readableState;rs.reading=false;if(rs.needReadable||rs.length<rs.highWaterMark){this._read(rs.highWaterMark);}}function Transform(options){if(!(this instanceof Transform))return new Transform(options);Duplex.call(this,options);this._transformState={afterTransform:afterTransform.bind(this),needTransform:false,transforming:false,writecb:null,writechunk:null,writeencoding:null};this._readableState.needReadable=true;this._readableState.sync=false;if(options){if(typeof options.transform==='function')this._transform=options.transform;if(typeof options.flush==='function')this._flush=options.flush;}this.on('prefinish',prefinish);}function prefinish(){var _this=this;if(typeof this._flush==='function'){this._flush(function(er,data){done(_this,er,data);});}else{done(this,null,null);}}Transform.prototype.push=function(chunk,encoding){this._transformState.needTransform=false;return Duplex.prototype.push.call(this,chunk,encoding);};Transform.prototype._transform=function(chunk,encoding,cb){throw new Error('_transform() is not implemented');};Transform.prototype._write=function(chunk,encoding,cb){var ts=this._transformState;ts.writecb=cb;ts.writechunk=chunk;ts.writeencoding=encoding;if(!ts.transforming){var rs=this._readableState;if(ts.needTransform||rs.needReadable||rs.length<rs.highWaterMark)this._read(rs.highWaterMark);}};Transform.prototype._read=function(n){var ts=this._transformState;if(ts.writechunk!==null&&ts.writecb&&!ts.transforming){ts.transforming=true;this._transform(ts.writechunk,ts.writeencoding,ts.afterTransform);}else{ts.needTransform=true;}};Transform.prototype._destroy=function(err,cb){var _this2=this;Duplex.prototype._destroy.call(this,err,function(err2){cb(err2);_this2.emit('close');});};function done(stream,er,data){if(er)return stream.emit('error',er);if(data!=null)stream.push(data);if(stream._writableState.length)throw new Error('Calling transform done when ws.length != 0');if(stream._transformState.transforming)throw new Error('Calling transform done when still transforming');return stream.push(null);}
...
const{parseUrl}=__webpack_require__(8);const Profiler=__webpack_require__(34);const Logger=__webpack_require__(35);const MySQL=__webpack_require__(36);const{typeCast,safeInvoke,prepareLegacyQuery,sanitizeInput}=__webpack_require__(89);const logger=new Logger(global.GetConvar('mysql_debug_output','console'));const profiler=new Profiler(logger,{trace:global.GetConvarInt('mysql_debug',0),slowQueryWarningTime:global.GetConvarInt('mysql_slow_query_warning',100)});const config=JSON.parse(global.LoadResourceFile('ghmattimysql','config.json'));const configFromString=parseUrl(global.GetConvar('mysql_connection_string','mysql://localhost/fivem'));const mysql=new MySQL(config||configFromString,logger,profiler);const currentResourceName=global.GetCurrentResourceName();global.exports('scalar',(query,parameters,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sql=query;let values=parameters;let cb=callback;[sql,values,cb]=sanitizeInput(sql,values,cb);mysql.execute({sql,values,typeCast},invokingResource).then(result=>{safeInvoke(cb,result&&result[0]?Object.values(result[0])[0]:null);});});global.exports('execute',(query,parameters,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sql=query;let values=parameters;let cb=callback;[sql,values,cb]=sanitizeInput(sql,values,cb);mysql.execute({sql,values,typeCast},invokingResource).then(result=>{safeInvoke(cb,result);});});function onTransactionError(error,connection,callback){connection.rollback(()=>{logger.error(error.message);safeInvoke(callback,false);});}global.exports('transaction',(querys,parameters,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sqls=[];let params=parameters;let cb=callback;if(!querys.every(element=>typeof element==='string')){sqls=querys;if(typeof parameters==='function')cb=parameters;}else{if(typeof parameters==='function'){cb=parameters;params=[];}querys.forEach(element=>{sqls.push({query:element,parameters:params});});}sqls.forEach((element,index)=>{const[stmt,stmtParams]=prepareLegacyQuery(element.query,element.parameters);sqls[index]={query:stmt,parameters:Array.isArray(stmtParams)?stmtParams:[]};});mysql.pool.getConnection((connectionError,connection)=>{if(connectionError){logger.error(connectionError.message);safeInvoke(cb,false);return;}connection.beginTransaction(transactionError=>{if(transactionError){onTransactionError(transactionError,connection,callback);return;}const promises=[];sqls.forEach(element=>{promises.push(mysql.execute({sql:element.query,values:element.parameters},invokingResource,connection));});Promise.all(promises).then(()=>{connection.commit(commitError=>{if(commitError){onTransactionError(commitError,connection,callback);}else safeInvoke(cb,true);});}).catch(executeError=>{onTransactionError(executeError,connection,callback);});});});});global.onNet(`${currentResourceName}:request-data`,()=>{const src=global.source;global.emitNet(`${currentResourceName}:update-resource-data`,src,profiler.profiles.resources);global.emitNet(`${currentResourceName}:update-time-data`,src,profiler.profiles.executionTimes);global.emitNet(`${currentResourceName}:update-slow-queries`,src,profiler.profiles.slowQueries);});
...
const profilerDefaultConfig={trace:false,slowQueryWarningTime:100,slowestQueries:21,timeInterval:300000};function updateExecutionTimes(object,queryTime){let returnObj=null;if(object){const{totalExecutionTime,queryCount}=object;returnObj={totalExecutionTime:totalExecutionTime+queryTime,queryCount:queryCount+1};}else{returnObj={totalExecutionTime:queryTime,queryCount:1};}return returnObj;}class Profiler{constructor(logger,config){this.version='MySQL';this.startTime=Date.now();this.logger=logger;this.config=Object.assign({},profilerDefaultConfig,config);this.profiles={executionTimes:[],resources:{},slowQueries:[]};this.slowQueryLimit=0;}get fastestSlowQuery(){return this.profiles.slowQueries.reduce((acc,cur)=>cur<acc?cur:acc);}addSlowQuery(sql,resource,queryTime){this.profiles.slowQueries.push({sql,resource,queryTime});if(this.profiles.slowQueries.length>this.config.slowestQueries){const min=this.fastestSlowQuery;this.profiles.slowQueries=this.profiles.slowQueries.filter(el=>el!==min);this.slowQueryLimit=this.fastestSlowQuery;}}setVersion(version){this.version=version;}profile(time,sql,resource){const interval=Math.floor((Date.now()-this.startTime)/this.config.timeInterval);const queryTime=time[0]*1e3+time[1]*1e-6;this.profiles.resources[resource]=updateExecutionTimes(this.profiles.resources[resource],queryTime);this.profiles.executionTimes[interval]=updateExecutionTimes(this.profiles.executionTimes[interval],queryTime);for(let i=0;i<interval;i+=1){if(!this.profiles.executionTimes[i]){this.profiles.executionTimes[i]={totalExecutionTime:0,queryCount:0};}}if(this.slowQueryLimit<queryTime){this.addSlowQuery(sql,resource,queryTime);}if(this.slowQueryWarningTime<queryTime){this.logger.error(`[${this.version}] [Slow Query Warning] [${resource}] [${queryTime.toFixed()}ms] ${sql}`);}if(this.config.trace){this.logger.log(`[${this.version}] [${resource}] [${queryTime.toFixed()}ms] ${sql}`);}}}module.exports=Profiler;

/***/ }),
/* 35 */
/***/ (function(module, exports, __webpack_require__) {

const{createWriteStream}=__webpack_require__(15);class Logger{constructor(output){this.output=output;this.fileStream=null;if(this.output==='file'||this.output==='both'){this.fileStream=createWriteStream(`./${global.GetCurrentResourceName()}.log`);}this.writeConsole=msg=>{console.log(msg);};}writeFile(msg){this.fileStream.write(`${msg}\n`);}log(msg){switch(this.output){default:case'console':this.writeConsole(msg);break;case'file':this.writeFile(msg);break;case'both':this.writeConsole(msg);this.writeFile(msg);break;}}error(msg){let errorMsg=msg;if(this.output==='console'){errorMsg=`\x1b[31m${msg}\x1b[0m`;}this.log(errorMsg);}}module.exports=Logger;

/***/ }),
/* 36 */
/***/ (function(module, exports, __webpack_require__) {

const mysql=__webpack_require__(16);class MySQL{constructor(mysqlConfig,logger,profiler){this.pool=null;this.profiler=profiler;this.logger=logger;this.formatQuery=sql=>`${sql.sql} : ${JSON.stringify(sql.values)}`;if(typeof mysqlConfig==='object'){this.pool=mysql.createPool(mysqlConfig);}else{this.logger.error(`[ERROR] [MySQL] Unexpected configuration of type ${typeof mysqlconfig} received.`);}this.pool.query('SELECT VERSION()',(error,result)=>{let versionPrefix='MariaDB';if(!error){const version=result[0]['VERSION()'];if(version[0]==='5'||version[0]==='8'){versionPrefix='MySQL';}profiler.setVersion(`${versionPrefix}:${version}`);logger.log('\x1b[32m[ghmattimysql]\x1b[0m Database server connection established.');}else{logger.error(`[ERROR] ${error.message}`);}});}execute(sql,invokingResource,connection){const queryPromise=new Promise((resolve,reject)=>{const start=process.hrtime();const db=connection||this.pool;db.query(sql,(error,result)=>{this.profiler.profile(process.hrtime(start),this.formatQuery(sql),invokingResource);if(error)reject(error);resolve(result);});});queryPromise.catch(error=>{this.logger.error(`[ERROR] [${this.profiler.version}] [${invokingResource}] An error happens on MySQL for query "${this.formatQuery(sql)}": ${error.message}`);});return queryPromise;}}module.exports=MySQL;
...
module.exports=ComQueryPacket;function ComQueryPacket(sql){this.command=0x03;this.sql=sql;}ComQueryPacket.prototype.write=function(writer){writer.writeUnsignedNumber(1,this.command);writer.writeString(this.sql);};ComQueryPacket.prototype.parse=function(parser){this.command=parser.parseUnsignedNumber(1);this.sql=parser.parsePacketTerminatedString();};
...
module.exports=ComStatisticsPacket;function ComStatisticsPacket(){this.command=0x09;}ComStatisticsPacket.prototype.write=function(writer){writer.writeUnsignedNumber(1,this.command);};ComStatisticsPacket.prototype.parse=function(parser){this.command=parser.parseUnsignedNumber(1);};
...
module.exports=OldPasswordPacket;function OldPasswordPacket(options){options=options||{};this.scrambleBuff=options.scrambleBuff;}OldPasswordPacket.prototype.parse=function(parser){this.scrambleBuff=parser.parsePacketTerminatedBuffer();};OldPasswordPacket.prototype.write=function(writer){writer.writeBuffer(this.scrambleBuff);};
...
"use strict";
module.exports=PassThrough;var Transform=__webpack_require__(26);var util=__webpack_require__(6);util.inherits=__webpack_require__(7);util.inherits(PassThrough,Transform);function PassThrough(options){if(!(this instanceof PassThrough))return new PassThrough(options);Transform.call(this,options);}PassThrough.prototype._transform=function(chunk,encoding,cb){cb(null,chunk);};
