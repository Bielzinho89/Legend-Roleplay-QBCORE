...
function Property:RegisterPropertyEntrance()
    ...
    local function showData()
        ...
        local content = "**Dono:** " .. data.owner .. "  \n" .. "**Descrição:** " .. data.description .. "  \n" .. "**Rua:** " .. data.street .. "  \n" .. "**Região:** " .. data.region .. "  \n" .. "**Interior:** " .. data.shell .. "  \n" .. "**À Venda:** " .. (data.for_sale and "Sim" or "Não")
...
function Property:ManageAccessMenu()
    ...
    if not self.owner then
        Framework[Config.Notify].Notify("Apenas o proprietário pode fazer isto.", "error")
        return
    end
    ...
    menu.options[#menu.options + 1] = {
        title = "Remover acesso de alguém",
        onSelect = function()
...
function Property:OpenDoorbellMenu()
    ...
    if not next(self.doorbellPool) then
        Framework[Config.Notify].Notify("Não tem ninguém na porta", "error")
        return
    end
    ...
    local menu = {
        id = id,
        title = "Há alguém tocando a campainha",
        options = {},
    }
...
function Property:LoadFurniture(furniture)
    ...
    if furniture.type == 'door' and Config.DynamicDoors then
        Debug("Objeto: "..furniture.label.." não será congelado")
    else
