...
AddEventHandler("ps-housing:server:registerProperty", function (propertyData, preventEnter) -- triggered by realtor job
    ...
    if propertyData.apartment and not preventEnter then
        ...
        if result and result[1] then
            Debug("Jogador: " .. propertyData.owner .. " a skin já existe!")
        else
            TriggerClientEvent("qb-clothes:client:CreateFirstCharacter", src)
            Debug("Jogador: " .. propertyData.owner .. " está criando um novo personagem!")
        end

        Framework[Config.Notify].Notify(src, "Abra o menu radial para o menu de mobílias e coloque seu baú e armário de roupas.", "info")
...
RegisterNetEvent("ps-housing:server:createNewApartment", function(aptLabel)
    ...
    Debug("Criando novo apartamento para " .. GetPlayerName(src) .. " in " .. apartment.label)

    Framework[Config.Logs].SendLog("Criando novo apartamento para " .. GetPlayerName(src) .. " em " .. apartment.label)
...
RegisterNetEvent("QBCore:Server:OnPlayerLoaded", function()
    ...
    if result and result[1] then
        Debug("Jogador: " .. citizenid .. " a skin já existe!")
    else
        TriggerClientEvent("qb-clothes:client:CreateFirstCharacter", src)
        Debug("Jogador: " .. citizenid .. " está criando um novo personagem!")
    end
end)
...
AddEventHandler("ps-housing:server:addTenantToApartment", function (data)
    ...
    for _, v in pairs(PropertiesTable) do
        ...
        if propertyData.owner == targetCitizenid then
            if propertyData.apartment == apartment then
                Framework[Config.Notify].Notify(targetSrc, "Você já está neste apartamento", "error")
                Framework[Config.Notify].Notify(targetSrc, "Esta pessoa já está neste apartamento", "error")
    ...
    if property_id == nil then
        ...
        Debug("Criando novo apartamento para " .. GetPlayerName(targetSrc) .. " em " .. newApartment.label)

        Framework[Config.Logs].SendLog("Criando novo apartamento para " .. GetPlayerName(targetSrc) .. " em " .. newApartment.label)

        Framework[Config.Notify].Notify(targetSrc, "Seu apartamento está agora em "..apartment, "success")
        Framework[Config.Notify].Notify(realtorSrc, "Você foi adicionado ".. targetToAdd.charinfo.firstname .. " " .. targetToAdd.charinfo.lastname .. " ao apartamento "..apartment, "success")
    ...
    Framework[Config.Notify].Notify(targetSrc, "Seu apartamento está agora em "..apartment, "success")
    Framework[Config.Notify].Notify(realtorSrc, "Você foi adicionado ".. targetPlayer.charinfo.firstname .. " " .. targetPlayer.charinfo.lastname .. " ao apartamento "..apartment, "success")
end)
...
function GetCitizenid(targetSrc, callerSrc)
    ...
    if not Player then
        Framework[Config.Notify].Notify(callerSrc, "Jogador não encontrado.", "error")
        return
    end
